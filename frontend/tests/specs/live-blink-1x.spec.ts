...
test('live blink at 100% zoom: neutral, mid, end', async ({ page }) => {
  await page.goto('/');
  ...
  await crow.screenshot({ path: path.join(outDir, 'blink-live-neutral-100.png') });
  ...
  await crow.screenshot({ path: path.join(outDir, 'blink-live-mid-100.png') });

  await page.waitForTimeout(200);
  await crow.screenshot({ path: path.join(outDir, 'blink-live-end-100.png') });
});
