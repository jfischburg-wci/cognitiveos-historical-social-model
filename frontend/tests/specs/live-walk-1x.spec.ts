...
test('live walk at 100% zoom: neutral, mid, end', async ({ page }) => {
  await page.goto('/');
  ...
  await crow.screenshot({ path: path.join(outDir, 'walk-live-neutral-100.png') });
  const cta = page.locator('section.cta');
  await cta.getByRole('button', { name: 'Walk', exact: true }).click();
  await page.waitForTimeout(320);
  await crow.screenshot({ path: path.join(outDir, 'walk-live-mid-100.png') });
  await page.waitForTimeout(900);
  await crow.screenshot({ path: path.join(outDir, 'walk-live-end-100.png') });
});
