<script>
  ...
  // Global caw function plays sound or synth fallback
  async function caw() {
    ...
    // Dispatch window event to animate beak in CrowRig
    try { window.dispatchEvent(new CustomEvent('corvid-caw')); } catch {}
  }

  // Lightweight synthetic crow sound (used only if audio fails)
  ...
  // CrowRig ready callback to set API
  function onCrowReady(e) {
    rig = e.detail.api;
  }

  // React to events via the LLM brain
...
<main class="page" in:fly={{ y: 24, duration: 700 }}>
  ...
  <!-- Interactive region with keyboard and pointer handlers -->
  <div
    class="surface"
    role="button"
    tabindex="0"
...
<style>
  ...
  .brand {
    ...
    height: auto;
    filter: drop-shadow(0 10px 40px rgba(130, 247, 255, 0.2));
  }
  .tag {
    opacity: 0.8;
    margin: 0.5rem 0 0;
  ...
  .feature {
    ...
    background: linear-gradient(180deg, rgba(130, 247, 255, 0.05), rgba(255, 255, 255, 0.02));
  }
  .feature h3 {
